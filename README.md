...
**llamactl**은 `llama.cpp`의 강력한 성능을 복잡한 파라미터 설정 없이 터미널에서 직관적으로 사용할 수 있게 해주는 Rust 기반의 TUI(Terminal User Interface) 컨트롤러입니다.
...
```bash
# 저장소 클론
git clone https://github.com/your-repo/llamactl.git
cd llamactl
...
```bash
# 대화형 TUI 실행
llamactl chat --model ./path/to/model.gguf
```
...
`llamactl`은 오픈소스 프로젝트입니다. 이슈 보고, 기능 제안, PR(Pull Request)은 언제나 환영합니다!
...
**llamactl**과 함께 로컬 LLM의 진정한 자유를 경험하세요! 🦙✨
